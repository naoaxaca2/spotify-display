...
リフレッシュトークンが失効している。Spotify のリフレッシュトークンには有効期限があり、長く使っていない端末はいずれこの状態になる。Developer Dashboard で Client Secret を再生成したり、アカウント設定でアプリの連携を解除した場合も同じ。

ログインボタンを押して認証し直せば復帰する。ローカル版・Pages 版とも、失効を検知すると保存済みトークンを自動で破棄してログイン画面に戻る仕組みになっている。

### プレイリストが「権限が不足しています」になる

トークンが古いスコープのまま。スコープを増やした版に更新したときに起きる。

ローカル版は `token_store.json` を削除、Pages 版は localStorage の `spotify_display_tokens` を削除して、ログインし直す。

### 再生ボタンを押しても何も起きない

操作対象の端末がない。どこかで Spotify アプリを開いて一度再生すれば解消する。SET タブの「再生する端末」で明示的に選ぶこともできる。

恒久的に解決したい場合は、Raspberry Pi 自身を再生端末にする方法を [RASPBERRY-PI.md](RASPBERRY-PI.md) に記載している。

### ログイン時に 400 エラーの画面が出る

画面に原因が表示される。よくあるのは次の3つ。

- **Client ID / Secret の不一致** — `.env` の値を確認する。クォートで囲んでいないか、前後に空白が混じっていないか
- **認証コードの期限切れ・使い回し** — エラー画面をリロードせず、`/` から入り直す
- **リダイレクト URI の不一致** — Developer Dashboard の登録内容を確認する

`.env` を書き換えたときは Flask の再起動が必要。デバッグモードの自動リロードは `.py` の変更しか見ていないため、`.env` だけ書き換えてもサーバは古い値を保持したままになる。

### Pages 版でブラウザが警告を出す

個人の `github.io` ドメインに置いた、有名サービスのログインを伴うページは、フィッシングと誤認されることがある。この構成では原理的に起こりうる。

まず [Google 透明性レポート](https://transparencyreport.google.com/safe-browsing/search) でドメインの状態を確認する。「データがありません」ならブロックリスト入りではないので、ブラウザの拡張機能か保護強化機能によるリアルタイム判定を疑う。実際に検出されている場合は、Safe Browsing の再審査を申請する。

### 認証情報を入れ替えるとき

Client Secret を再生成しただけでは、発行済みのリフレッシュトークンは無効にならない。漏洩に対処する場合は、あわせて[アカウント設定](https://www.spotify.com/account/apps/)からアプリの連携を解除する。

そのあと、ローカル版と Pages 版の両方で再ログインが必要になる。Pages 版は端末ごとに必要。

---

## ブラウザに保存されるデータ

Pages 版および設定まわりで使う localStorage のキー。

| キー | 内容 |
|---|---|
| `spotify_display_tokens` | Pages 版のアクセス／リフレッシュトークン |
| `spotify_display_settings` | 表示方法、スクロール方向、再生する端末 |
| `spotify_display_last_device` | 前回再生した端末 |
| `spotify_display_shuffle` | ランダム演奏の ON / OFF |

`spotify_display_pkce_verifier` と `spotify_display_pkce_state` は認証中のみ sessionStorage に置かれ、完了時に削除される。
